...
class MainActivity : AppCompatActivity(), OnMapReadyCallback, SuggestionAdapter.OnToggleClickListener, ActivitiesAdaptor.OnItemClickListener {
    ...
    private lateinit var beachselected:String
    lateinit var beachdata: JSONObject
    ...
    private fun fetchbeachdata() {
        Log.d("fetchbeachdata", "Entered fetchbeachdata()")
        val url = "http://www.beachbuddyprototype.freewebhostmost.com/$beachselected"
        ...
        jsonArrayRequest = JsonArrayRequest(
            ...
            { response ->
                try {
                    ...
                    Log.d("fetchbeachdata", "About to check for response.length() > 0")
                    if (response.length() > 0) {
                        Log.d("fetchbeachdata", "Entered response.length() > 0")
                        // Get the first object in the array
                        val firstObject = response.getJSONObject(0)

                        // Store the JSON object in the beachdata variable
                        beachdata = firstObject

                        // Fetch the "activities" array from beachdata
                    ...
                    Log.d("error","Exception JSON")
                    println("Error parsing beach data: ${e.message}")
                }
            },
            { error ->
                // Handle the error
                Log.d("error","Exception JSON")
                println("Error fetching beach data: ${error.message}")
            }
        )
    ...
    private fun initBeachesRecycler(){
        ...
        fun loadBeachesFromJson(){
            val drawableMap = mapOf(
                ...
                "beach_elliot" to R.drawable.beach_elliot,
                "beach_elephant" to R.drawable.beach_elephant,
                "beach_kozhikode" to R.drawable.beach_kozhikode,
                "beach_laxmanpur" to R.drawable.beach_laxmanpur,
                "beach_kala" to R.drawable.beach_kala,
                "beach_palolem" to R.drawable.beach_palolem,
                "beach_butterfly" to R.drawable.beach_butterfly,
                "beach_baga" to R.drawable.beach_baga,
                "beach_alappuzha" to R.drawable.beach_alappuzha,
                "beach_anjuna" to R.drawable.beach_anjuna,
                "beach_candolim" to R.drawable.beach_candolim,
                "beach_cherai" to R.drawable.beach_cherai,
                "beach_vijaynagar" to R.drawable.beach_vijaynagar,
                "beach_radhanagar" to R.drawable.beach_radhanagar,
                "beach_varkala" to R.drawable.beach_varkala
            )
            ...
            for (beachJson in beachJsonList) {
                ...
                beach.boundary = beachJson.polygonCoordinates
        ...
        BeachRecyclerView.adapter = BeachAdapter(BeachesList){ selectedBeach ->
            ...
            beachselected = BeachesList[0].imageName
            fetchbeachdata()
            showSuggestions(false)
    ...
    private fun searchPlaces(Places: MutableList<String>) {
        ...
        val newCameraPosition = CameraPosition(LatLng(coordinate[0], coordinate[1]), 8.0F, 0F, 0F)
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        ...
        mGoogleMap?.setOnMarkerClickListener { marker ->
            ...
            val newCameraPosition = CameraPosition(marker.position, 14.0F, 0F, 0F)
            CameraUpdateFactory.newCameraPosition(newCameraPosition)
            mGoogleMap!!.animateCamera(CameraUpdateFactory.newCameraPosition(newCameraPosition))
            fun randomReadings(){
                binding.includeweather.temperatureValue.text = "${Random.nextInt(30, 41)}°C"                               // Random temperature between -10°C and 40°C
                binding.includeweather.waveHeightValue.text = "${"%.1f".format(Random.nextDouble(0.5, 10.0))} m"            // Random wave height between 0.5 and 10 meters
                binding.includeweather.swellPerSecondsValue.text = "${Random.nextInt(1, 8)} "                               // Random swell period between 1 and 20 seconds
                binding.includeweather.windSpeedValue.text = "${Random.nextInt(0, 151)} KM/Hr"                              // Random wind speed between 0 and 150 KM/Hr
                binding.includeweather.oceanCurrentsValue.text = "${"%.1f".format(Random.nextDouble(0.0, 5.0))} Km/Hr"      // Random ocean currents between 0 and 5 Km/Hr
                binding.includeweather.windDirectionValue.text = "${Random.nextInt(0, 361)}°"                               // Random wind direction between 0° and 360°
                binding.includeweather.waterBacteriaLevelValue.text = "${"%.4f".format(Random.nextDouble(0.0001, 1.0))}%"   // Random water bacteria level between 0.0001% and 1%
                binding.includeweather.UVRadiationValue.text = "${Random.nextInt(0, 13)}"                                   // Random UV index between 0 and 12
            }

            randomReadings()

            beachselected = when (marker.title) {
                "Marina Beach" -> "beach_marina"
                "Kovalam" -> "beach_kovalam"
                "Varkala" -> "beach_varkala"
                else -> null.toString()
            }

            // Check if a valid beach was selected and take appropriate action
            beachselected.let {
                fetchbeachdata() // Keep the function call as is
            }
    ...
    private fun createPolygonsFromAnArray(beaches: List<Beach>) {
        for (beach in beaches) {
            ...
            for (coordinate in beach.boundary) {
                Log.d("createPolygonsFromAnArray", "Created polygon at ${coordinate[1]}, ${coordinate[0]}")
    ...
    private fun updateMapLocation(location: Location) {
        ...
        mGoogleMap?.moveCamera(CameraUpdateFactory.newLatLngZoom(currentLoc, 14.0F))
    }
    ...
    private fun addMarkersForCategory(category: String) {
        Log.d("addMarkersForCategory", "Entered addMarkersForCategory()")
        fetchbeachdata()
        Log.d("addMarkersForCategory", "just after fetched Data()")
        when (category) {
            "Hotels" -> {
                // Fetch the 'hotels' data from beachdata JSON (assuming 'hotels' is a key in JSON)
                val hotelsArray = beachdata.getJSONArray("hotels")

                // Convert JSON data into an array of LatLng and name pairs
                val hotels = (0 until hotelsArray.length()).mapNotNull { i ->
                    val hotel = hotelsArray.getJSONObject(i)
                    val lat = hotel.getDouble("latitude")
                    val lng = hotel.getDouble("longitude")
                    val name = hotel.getString("name")

                    // Create LatLng object and pair it with the hotel name
                    LatLng(lat, lng) to name
                }
                ...
                val markers = hotels.mapNotNull { (location, title) ->
                    mGoogleMap?.addMarker(
                        MarkerOptions()
                            .position(location)
                            .title(title)
                            .icon(BitmapDescriptorFactory.fromBitmap(hotelIcon))
                    )
                }.toMutableList()
            ...
            "Hospitals & Clinics" -> {
                val hotelsArray = beachdata.getJSONArray("hospitals")

                // Convert JSON data into an array of LatLng and name pairs
                val loc = (0 until hotelsArray.length()).mapNotNull { i ->
                    val hotel = hotelsArray.getJSONObject(i)
                    val lat = hotel.getDouble("latitude")
                    val lng = hotel.getDouble("longitude")
                    val name = hotel.getString("name")
                ...
                val markers = loc.mapNotNull { (location, title) ->
                    mGoogleMap?.addMarker(
                        MarkerOptions()
                            .position(location)
                            .title(title)
                            .icon(BitmapDescriptorFactory.fromBitmap(hospitalIcon))
                    )
                }.toMutableList()
            ...
            "Toilets" -> {
                val hotelsArray = beachdata.getJSONArray("toilets")
                ...
                val markers = loc.mapNotNull { (location, title) ->
                    mGoogleMap?.addMarker(
                        MarkerOptions()
                            .position(location)
                            .title(title)
                            .icon(BitmapDescriptorFactory.fromBitmap(toiletIcon))
                    )
                }.toMutableList()
            ...
            "Drinking Water" -> {
                Log.d("addMarkersForCategory", "Trying for Drinking Water")
                val hotelsArray = beachdata.getJSONArray("drinking_water")
                Log.d("addMarkersForCategory", "Fetched array of Drinking Water of length : ${hotelsArray.length()}")
                ...
                val loc = (0 until hotelsArray.length()).mapNotNull { i ->
                    ...
                    val lat = hotel.getDouble("latitude")
                    val lng = hotel.getDouble("longitude")
                    Log.d("addMarkersForCategory", "Fetched drinking water's Lng as ${lng}")
                    val name = hotel.getString("name")
                    Log.d("addMarkersForCategory", "Fetched drinking water's name as ${name}")

                    // Create LatLng object and pair it with the hotel name
                    LatLng(lat, lng) to name
                }

                Log.d("addMarkersForCategory", "Converted JSON data into an array")
                ...
                val markers = loc.mapNotNull { (location, title) ->
                    mGoogleMap?.addMarker(
                        MarkerOptions()
                            .position(location)
                            .title(title)
                            .icon(BitmapDescriptorFactory.fromBitmap(dinkingwaterIcon))
                    )
                }.toMutableList()
                Log.d("addMarkersForCategory", "Added marker to the map")
            ...
            "Restaurants" -> {
                val hotelsArray = beachdata.getJSONArray("restaurants")
                ...
                val markers = loc.mapNotNull { (location, title) ->
                    mGoogleMap?.addMarker(
                        MarkerOptions()
                            .position(location)
                            .title(title)
                            .icon(BitmapDescriptorFactory.fromBitmap(restaurantsIcon))
                    )
                }.toMutableList()
            ...
            "ATMS" -> {
                val hotelsArray = beachdata.getJSONArray("atms")
                ...
                val markers = loc.mapNotNull { (location, title) ->
                    mGoogleMap?.addMarker(
                        MarkerOptions()
                            .position(location)
                            .title(title)
                            .icon(BitmapDescriptorFactory.fromBitmap(ATMSIcon))
                    )
                }.toMutableList()
            ...
            "Parking" -> {
                val hotelsArray = beachdata.getJSONArray("parking")
                ...
                val markers = loc.mapNotNull { (location, title) ->
                    mGoogleMap?.addMarker(
                        MarkerOptions()
                            .position(location)
                            .title(title)
                            .icon(BitmapDescriptorFactory.fromBitmap(parkingIcon))
                    )
                }.toMutableList()
...
//        Adding title to polygon
    ...
    private fun initProhibitedSubcategory(){
        ...
        prohibitedActivities = mutableListOf("Bathing", "Surfing", "Fireworks")
    ...
    private fun bottomSheet_alert() {
        ...
        fun initHelpButton(button: ImageButton, phoneNumber: Long) {
            ...
            button.setBackgroundResource(R.drawable.phone_background)
        ...
        initHelpButton(binding.includealert.ambulanceButton,102)
        initHelpButton(binding.includealert.policeButton, 100)
        initHelpButton(binding.includealert.womanButton, 1091)
        initHelpButton(binding.includealert.disasterManagementButton, 1078)
        initHelpButton(binding.includealert.touristHelplineButton, 1363)
        initHelpButton(binding.includealert.nationalDisasterResponseForceButton, 9711077372)
    }
    ...
    override fun onToggleClick(position: Int, isChecked: Boolean) {
        ...
        when (position) {
            ...
            5 -> updateMarkers(isChecked, "Toilets")
            6 -> updateMarkers(isChecked, "Drinking Water")
            7 -> updateMarkers(isChecked, "Restaurants")
        }
    }
            ...
            for (i in 1..20) {
                mockListOfRandomNotifications.add(
                    NotificationClass().apply {
                        ...
                        message = when (type) {
                            1 -> "Category 3 Hurricanee is expected at your planned location"
